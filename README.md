...
```sh
make docker-build docker-push IMG=<some-registry>/ferron-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/ferron-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/ferron-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/ferron-operator/<tag or branch>/dist/install.yaml
```
